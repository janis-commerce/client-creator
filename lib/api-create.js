...
module.exports = class ClientCreateAPI extends API {
	...
	get struct() {
		return struct.partial({
			...
			processClients: 'boolean?'
		});
	}

	async process() {

		const { clients: clientsToCreate, processClients } = this.data;

		if(clientsToCreate)
			return this.createClients(clientsToCreate);

		return processClients && this.processClients();
	}
	...
	async processClients() {
		...
		this.setBody({
			...createdClients && { createdClients: getClientsCodes(createdClients) },
			...updatedClients && { updatedClients: getClientsCodes(updatedClients) },
			...removedClients && { removedClients }
		});

		return Promise.all([
			createdClients && this.postSaveHook(getClientsCodes(createdClients), createdClients),
