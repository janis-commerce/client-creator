...
module.exports = class Remover extends Base {
	...
	static async remove(clientsCodes) {
		...
		if(!currentClients.length)
			return logger.info(`Unable to get clients from ${this.serviceName}, they won\`t be removed.`);

		if(currentClients.length !== clientsCodes.length)
			logger.warn(`Some clients could\`t be obtained from ${this.serviceName}, they won\`t be removed.`);
